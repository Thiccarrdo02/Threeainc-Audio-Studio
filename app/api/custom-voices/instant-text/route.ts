  ...
  try {
    ...
    if (text.length < 100 || text.length > 1000) {
      return errorResponse(
        ...
        "Target text must be between 100 and 1000 characters for ElevenLabs reference-audio generation.",
      );
    }
    ...
    return errorResponse(
      502,
      "ELEVENLABS_INSTANT_TEXT_FAILED",
      error instanceof Error
        ? error.message
        : "Instant reference voice generation failed.",
    );
  }
}
